Shader "FX/Hologram Shader"
{
	Properties
	{
		...
		_ScrollSpeedV("Alpha scroll Speed", Range(0, 10.0)) = 1.0
			// Glow
			_GlowIntensity("Glow Intensity", Range(0.01, 10.0)) = 0.5
			// Glitch
			_GlitchSpeed("Glitch Speed", Range(0, 100)) = 50.0
			...
			Pass
			{
				...
				CGPROGRAM
					...
					v2f vertexFunc(appdata IN) {
						...
						//Glitch
						IN.vertex.z += cos(_Time.y * _GlitchSpeed * 5 * IN.vertex.y) * _GlitchIntensity;
						IN.vertex.y += cos(_Time.y * _GlitchSpeed * 5 * IN.vertex.y) * _GlitchIntensity;
						IN.vertex.x += cos(_Time.y * _GlitchSpeed * 5 * IN.vertex.y) * _GlitchIntensity;
					...
					fixed4 fragmentFunc(v2f IN) : SV_Target{
						...
						pixelColor.w = (alphaColor.w + 0.5f) / 1.5f;

						// Rim Light
						//half rim = 1.0 - saturate(dot(IN.viewDir, IN.worldNormal));

						return pixelColor * _Color * _GlowIntensity;
					}
				ENDCG
			}
		}
